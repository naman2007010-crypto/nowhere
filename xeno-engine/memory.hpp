...
namespace memory {
    ...
    // Template function to read any type from memory
    template<typename T>
    ...
    inline uintptr_t ReadPointerChain(HANDLE hProcess, uintptr_t base, const std::vector<uintptr_t>& offsets) {
    ...
    inline std::string ReadRobloxString(HANDLE hProcess, uintptr_t stringPtr) {
        ...
        // Roblox strings: length at offset 0x10, data at offset 0x18
    ...
    inline std::string ReadString(HANDLE hProcess, uintptr_t address, size_t maxLen = 256) {
        std::string result;
    ...
    template<typename T>
    inline std::vector<T> ReadArray(HANDLE hProcess, uintptr_t address, size_t count) {
        std::vector<T> result(count);
        ReadProcessMemory(hProcess, (LPCVOID)address, result.data(), count * sizeof(T), nullptr);
        return result;
    }
    ...
    inline std::array<float, 16> ReadMatrix4x4(HANDLE hProcess, uintptr_t address) {
        ...
        ReadProcessMemory(hProcess, (LPCVOID)address, matrix.data(), sizeof(float) * 16, nullptr);
        return matrix;
    }
